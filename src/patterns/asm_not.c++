...
class asm_not: public libcgraph::pattern {
private:
    std::shared_ptr<libcgraph::node> in, out;

public:
    asm_not(size_t _width)
    ...
    void step(ssize_t cycle)
        {
            out->update(0 - in->value() - 1, cycle);
        }
};

class asm_not_factory: public libcgraph::pattern_factory {
    ...
    const pattern_ptr create(const std::string& args) const
        {
            ...
            return std::make_shared<asm_not>(width);
        }
...
void cons(void)
{
    auto factory = std::make_shared<asm_not_factory>();
