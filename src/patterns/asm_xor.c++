...
class asm_xor: public libcgraph::pattern {
private:
    std::shared_ptr<libcgraph::node> inA, inB, out;

public:
    asm_xor(size_t _width)
...
class asm_xor_factory: public libcgraph::pattern_factory {
    ...
    const pattern_ptr create(const std::string& args) const
        {
            ...
            return std::make_shared<asm_xor>(width);
        }
...
void cons(void)
{
    auto factory = std::make_shared<asm_xor_factory>();
