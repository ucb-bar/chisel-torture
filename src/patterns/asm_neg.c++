...
class asm_neg: public libcgraph::pattern {
private:
    std::shared_ptr<libcgraph::node> in, out;

public:
    asm_neg(size_t _width)
    ...
    void step(ssize_t cycle)
        {
            out->update(0 - in->value(), cycle);
        }
};

class asm_neg_factory: public libcgraph::pattern_factory {
    ...
    const pattern_ptr create(const std::string& args) const
        {
            ...
            return std::make_shared<asm_neg>(width);
        }
...
void cons(void)
{
    auto factory = std::make_shared<asm_neg_factory>();
